  ...
  <div class="container">
    ...
    <button  @click="apiCall">Search</button>
      ...
      <p class="temp">{{isSet?Math.round(weather.list[0].main.temp)+"°C":"No temp"}}</p>
      
      <img :src="getImgURL()" alt="Icon could not be loaded">

      
      <p><b>{{isSet?weather.list[0].weather[0].main:""}}</b></p>
...
export default{
    ...
    methods:{
        ...
        apiCall(){
            ...
            //this.isSet = true; 
            document.getElementById('infoDiv').style.display = "block"; 
            this.icon= "https://openweathermap.org/img/wn/"+this.weather.list[0].weather[0].icon +".png";
            this.icon=true;
            //this.weather = data2; 
            //console.log(data2);
